# stgit - A Simple CLI for Stacked Git Branches

**`stgit`** is a command-line tool written in Bash that simplifies working with stacked Git branches. Inspired by the workflows of tools like Graphite, it helps you create, manage, and submit dependent chains of branches without the usual hassle of manual rebasing and pull request management.

It works by maintaining a simple set of metadata in your local Git configuration, allowing it to understand the relationships between your branches. When combined with the GitHub CLI (`gh`), `stgit` can automate large parts of your development workflow.
...
`stgit` treats a series of dependent branches as a "stack". When you run `stgit create <branch-name>`, it creates a new branch and records its parent in your local Git config (e.g., `branch.branch-name.parent=parent-branch`).

This simple parent-child metadata is the foundation for all of `stgit`'s powerful features. It allows the tool to traverse the stack, understand dependencies, and perform complex operations like rebasing the entire chain with a single command.
...
2.  **Download the script**: Save the `stgit.sh` file to your computer.

3.  **Make it executable**:

    ```bash
    chmod +x /path/to/stgit.sh
    ```

4.  **Place it in your PATH**: For easy access, move the script to a directory in your system's `PATH`.

    ```bash
    # For example:
    sudo mv /path/to/stgit.sh /usr/local/bin/stgit
    ```
...
### Stack & Branch Management
  ...
  * #### `stgit insert [--before] <branch-name>`

    Inserts a new branch into the stack. By default, it inserts *after* the current branch. The `--before` flag inserts it *before* the current branch. `stgit` will automatically rebase any descendant branches and update their associated pull requests on GitHub.

    ```bash
    # In a stack main -> feature-a -> feature-c
    # You are on 'feature-a'
    stgit insert feature-b

    # The new stack is main -> feature-a -> feature-b -> feature-c
    # and you are now on 'feature-b'.
    ```

  * #### `stgit squash [--into parent|child]`

    Squashes the commits from one branch into another and deletes the squashed branch. By default, it squashes the current branch into its `parent`. Use `--into child` to squash the child branch into the current one. `stgit` will prompt to close the pull request of the deleted branch.

  * #### `stgit delete`

    Deletes the current branch both locally and remotely. If the branch has children, they are re-parented and rebased onto the deleted branch's parent. `stgit` will also prompt you to close the associated pull request on GitHub.

  * #### `stgit track <set|remove> [parent-branch]`
    ...
    Manually manages stack metadata.
      ...
      * `track remove`: Removes `stgit` metadata from the current branch.
...
### Synchronization & History
  ...
  * #### `stgit amend`
    ...
    A convenient shortcut. It adds all staged changes to the most recent commit (`git commit --amend --no-edit`) and then automatically runs `stgit restack` if the current branch has children.
...
```bash
# 1. Check the status. `stgit` will fetch from the remote and detect changes.
stgit status
...
# 💡 Next step: Run 'stgit sync' to clean up and rebase the stack.
...
# 🟢 Operation complete.
# 💡 Next step: Run 'stgit push' to update your remote branches.
...
# 💡 Next step: Run 'stgit push' to update the remote.
...
# Alternatively, you can just run `stgit amend` after adding the file.
